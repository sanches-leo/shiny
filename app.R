...
ui <- navbarPage(
    "LACEN Pipeline",
    id = "main_nav",
    tabPanel("Greetings", value = "greetings",
             fluidPage(
    ...
    tabPanel("Filter and Transform", value = "filter_transform",
             fluidPage(
    ...
    tabPanel("Soft Threshold", value = "soft_threshold",
             fluidPage(
    ...
    tabPanel("Summarize and Enrich", value = "summarize_enrich",
             fluidPage(
    ...
    tabPanel("LNC-centric Analysis", value = "lnc_centric",
             fluidPage(
...
values$lacenObject <- initLacen(annotationData = annotation_data,
    ...
    observeEvent(input$check_data_btn, {
        if (!is.null(values$lacenObject)) { # Demo data path
            ...
            req(input$annotationData_file, input$expressionDGEData_file, input$ncAnnotation_file, 
            ...
            values$lacenObject <- initLacen(annotationData = annotationData,
        ...
        if (isTRUE(check_result)) {
            ...
            output$check_data_output <- renderPrint({ "Data check passed! Proceeding to the next step." })
            updateNavbarPage(session, "main_nav", selected = "filter_transform")
        } else {
            output$check_data_output <- renderPrint({ check_result })
        }
    })
    ...
    observeEvent(input$run_filter_transform_btn, {
        ...
        values$lacenObject <- filterTransform(lacenObject = values$lacenObject,
    ...
    observeEvent(input$run_soft_threshold_btn, {
        req(values$lacenObject)
        values$lacenObject <- selectSoftThreshold(lacenObject = values$lacenObject,
                                                  indicePower = input$indicePower_input)
        updateNavbarPage(session, "main_nav", selected = "summarize_enrich")

        # Run summarize and enrich
        values$lacenObject <- summarizeAndEnrichModules(lacenObject = values$lacenObject,
                                                        maxBlockSize = 40000,
                                                        filename = "enrichedgraph.png")
        stackedBarplot(values$lacenObject, filename = "stackedplot.png", plot = FALSE)

        output$enriched_graph_plot <- renderImage({
            list(src = "enrichedgraph.png", contentType = 'image/png', alt = "Enriched Graph")
        }, deleteFile = TRUE)
        output$stacked_barplot_plot <- renderImage({
            list(src = "stackedplot.png", contentType = 'image/png', alt = "Stacked Barplot")
        }, deleteFile = TRUE)
    })
    ...
    observeEvent(input$run_heatmap_btn, {
        ...
        heatmapTopConnectivity(lacenObject = values$lacenObject,
    ...
    observeEvent(input$run_lnc_analysis_btn, {
        ...
        lncRNAEnrich(lncName = input$lncSymbol_input,
